...
DEFAULT_GELF_OUTPUT_TCP_RETRY="0"
...
LOGSTASH_NICE="9"
if [ -z "${LOGSTASH_START_OPTIONS}" ]; then
...
logstash_config() {
	if [ -z "${OUTPUT_PLUGIN}" ] ; then
		...
		if [ "${OUTPUT_PLUGIN}" == "kafka" ] ; then
			...
			if [ -z "${KAFKA_WORKERS}" ] ; then
				export KAFKA_WORKERS="${DEFAULT_KAFKA_WORKERS}"
			fi
			...
			if [ -z "${GELF_OUTPUT_TLS}" ] ; then
				...
				export GELF_OUTPUT_TLS="${DEFAULT_GELF_OUTPUT_TLS}"
                        ...
                        if [ -z "${GELF_OUTPUT_TCP_RETRY}" ] ; then
                                echo "WARNING no environmnet variable GELF_OUTPUT_TCP_RETRY is define. Use \"${DEFAULT_GELF_OUTPUT_TCP_RETRY}\" by default."
                                export GELF_OUTPUT_TCP_RETRY="${DEFAULT_GELF_OUTPUT_TCP_RETRY}"
                        fi
		...
		if [ "${OUTPUT_PLUGIN}" == "kafka" ] ; then
			...
			do
				if [[ $VAR =~ ^KAFKA_ && ! $VAR =~ ^KAFKA_HOME 	\
					...
					&& ! $VAR =~ ^KAFKA_BROKER_PORT ]]; then
					logstash_kafka_param=`echo "$VAR" | sed -r "s/KAFKA_(.*)=.*/\1/g" | tr '[:upper:]' '[:lower:]'`
					env_var=`echo "$VAR" | sed -r "s/(.*)=.*/\1/g"`
					output+=("	$logstash_kafka_param => \"${!env_var}\" \\n")
				fi
			done
			...
			output+=("    tls => \"${GELF_OUTPUT_TLS}\"\\n")
                        output+=("    tcp_retry => \"${GELF_OUTPUT_TCP_RETRY}\"\\n")
			output+=("  }\\n")
		fi
        fi
	if [ "x$DEBUG" == "x1" ] ; then
...
start_logstash () {
	...
	LS_JAVA_OPTIONS="${LS_JAVA_OPTIONS} -Djava.io.tmpdir=${LOGSTASH_HOME}"
	HOME=${LOGSTASH_HOME}
	export PATH HOME LS_HEAP_SIZE LS_JAVA_OPTS LOGSTASH_USE_GC_LOGGING
